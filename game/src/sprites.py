...
class Player(pygame.sprite.Sprite):
    ...
    def _update_animation(self, dt: float) -> None:
        ...
        if self.anim_timer >= speed:
            ...
            self.anim_frame += 1
        self.anim_state = ('jump' if self.velocity_y < 0 else 'fall') if not self.is_on_ground else ('run' if abs(self.velocity_x)>10 else 'idle')
...
class Boss(pygame.sprite.Sprite):
    ...
    def take_hit(self) -> bool:
        if not self.alive_flag or not self.stunned or self.flash_timer > 0:
            return False
        self.hp -= 1
